#!/usr/bin/env bash
# Anymal-C manager-based APGD run matching the validated Jacobi comparison:
# 500 PPO, contact APGD with 20 iterations, LDL bilateral joints,
# coupling_iterations=2, post_stabilize_joints=false, cache_factorization=true.
set -uo pipefail
source ~/miniforge3/etc/profile.d/conda.sh
conda activate dvi
cd ~/repos/isaaclab-dvi
ROOT="$(pwd)/results/rl_solver_sweep/raw/anymal_c/apgd_coupling2_cache_on_contact20"
LOG="$ROOT/run.log"
mkdir -p "$ROOT"
if [[ -f "$ROOT/COMPLETED" ]]; then echo "Already completed: $ROOT"; exit 0; fi
{
 echo "===== START anymal_c apgd_coupling2_cache_on_contact20 UTC $(date -u +%FT%TZ) ====="
 echo "task=Isaac-Velocity-Flat-Anymal-C-v0 preset=newton_dvi_apgd max_iterations=500 joint_solver=sparse_ldl contact_solver=sparse_apgd contact_iterations=20 coupling_iterations=2 post_stabilize_joints=false cache_factorization=true"
...
./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py \
 ...
 2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
echo "===== END anymal_c apgd_coupling2_cache_on_contact20 UTC $(date -u +%FT%TZ) exit=$status =====" | tee -a "$LOG"
