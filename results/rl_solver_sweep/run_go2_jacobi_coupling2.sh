...
for CONTACT_ITERS in 5 10 15; do
  OUT="$PWD/results/rl_solver_sweep/raw/go2/newton_dvi_coupling2/contact_${CONTACT_ITERS}"
  mkdir -p "$OUT"
  LOG="$OUT/run.log"
  ...
  {
    echo "===== START go2 newton_dvi_coupling2 UTC $(date -u +%FT%TZ) ====="
    echo "task=Isaac-Velocity-Flat-Unitree-Go2-v0 preset=newton_dvi max_iterations=300 contact_iterations=$CONTACT_ITERS coupling_iterations=2"
  ...
  ./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py \
    --task Isaac-Velocity-Flat-Unitree-Go2-v0 --headless --max_iterations 300 \
    presets=newton_dvi \
    ...
    2>&1 | tee -a "$LOG"
  status=${PIPESTATUS[0]}
  echo "===== END go2 newton_dvi_coupling2 contact_${CONTACT_ITERS} UTC $(date -u +%FT%TZ) exit=$status =====" | tee -a "$LOG"
