#!/usr/bin/env python3
"""Overlay all validated G1 reward curves with the coupling-2 Jacobi runs."""
...
OUT = ROOT / "figures"
RUNS = [
    ("Jacobi, 5", "newton_dvi", "contact_5", "#0000FF", "-", 2.0),
    ("APGD, 5", "newton_dvi_apgd", "contact_5", "#000000", "-", 1.8),
    ("P-SPG-FB, 5", "newton_dvi_pspg", "contact_5", "#FF0000", "-", 1.8),
    ("Jacobi, 10", "newton_dvi", "contact_10", "#0000FF", "--", 2.0),
    ("APGD, 10", "newton_dvi_apgd", "contact_10", "#000000", "--", 1.8),
    ("P-SPG-FB, 10", "newton_dvi_pspg", "contact_10", "#FF0000", "--", 1.8),
    ("MJWarp (native)", "newton_mjwarp", "contact_10", "#008000", "-", 2.0),
]
COUPLING_RUNS = [
    ("Jacobi, 5; coupling=2", 5, "#0000FF", ":"),
    ("Jacobi, 10; coupling=2", 10, "#0000FF", "-."),
    ("Jacobi, 15; coupling=2", 15, "#0000FF", (0, (6, 2, 1, 2))),
]
...
for label, solver, budget, color, style, width in RUNS:
    ...
    ax.fill_between(x, lo, hi, color=color, alpha=0.045, linewidth=0)
...
ax.set_xlim(left=0); ax.grid(True, alpha=0.22)
ax.legend(loc="best", fontsize=8, ncol=2)
OUT.mkdir(parents=True, exist_ok=True)
for suffix, kwargs in ((".pdf", {}), (".svg", {}), (".png", {"dpi": 220})):
    fig.savefig(OUT / f"g1_all_reward_curves_overlay{suffix}", **kwargs)
plt.close(fig)
print(OUT / "g1_all_reward_curves_overlay.pdf")
