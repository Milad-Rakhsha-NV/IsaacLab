...
class ClosedLoopView:
    ...
    def __init__(self, model: Model, pattern: str = "*") -> None:
        ...
        # ------------------------------------------------------------------
        # Assembled reference body state for closed-loop RESET.
        #
        # A closed kinematic loop has no spanning tree, so the standard reset path
        # (write root pose + joint_q, then FK to fill the rest) cannot reconstruct the
        # passive linkage bodies -- they get left at the origin with stale/NaN body_qd.
        # ``model.body_q`` holds the authored, loop-closed body poses (with per-env
        # offsets already applied by the cloner). We snapshot it here so reset can
        # restore ALL bodies rigidly relative to the desired root pose.
        self._has_assembled_reference = False
        self._assembled_body_q: wp.array | None = None  # (world_count, bodies_per_world) transformf
        try:
            if model.body_q is not None and self._bodies_per_world > 0:
                ref = wp.clone(model.body_q)
                self._assembled_body_q = ref.reshape((self._world_count, self._bodies_per_world))
                self._has_assembled_reference = True
        except Exception as exc:  # pragma: no cover - defensive
            logger.warning("ClosedLoopView: failed to snapshot assembled body_q for reset: %s", exc)
    ...
    # ------------------------------------------------------------------
    # Closed-loop reset support
    # ------------------------------------------------------------------

    @property
    def is_implicit_free_base(self) -> bool:
        """True when the root body has no joint to world (body_q[0] is the root pose)."""
        return self._is_implicit_free_base

    @property
    def has_assembled_reference(self) -> bool:
        """True when an assembled-pose snapshot is available for full-body reset."""
        return self._has_assembled_reference
    ...
    @property
    def assembled_body_q(self) -> wp.array | None:
        """Assembled per-world reference body poses ``(world_count, bodies_per_world)``.

        These are the authored, loop-closed body transforms captured from ``model.body_q``
        at construction time (env offsets already applied). Used by the articulation reset
        path to restore the full closed-loop body state rigidly relative to a new root pose.
        """
        return self._assembled_body_q

    def get_link_transforms_flat(self, source: Model | State) -> wp.array:
        """Flat ``(world_count * bodies_per_world,)`` view of body_q (no per-world axis)."""
        return source.body_q

    def get_link_velocities_flat(self, source: Model | State) -> wp.array:
        """Flat ``(world_count * bodies_per_world,)`` view of body_qd (no per-world axis)."""
        return source.body_qd
