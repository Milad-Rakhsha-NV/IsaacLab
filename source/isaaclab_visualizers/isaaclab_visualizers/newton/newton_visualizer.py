...
class NewtonVisualizer(BaseVisualizer):
    ...
    def is_reset_requested(self) -> bool:
        ...
        return self._viewer_ui_flag("is_reset_requested")

    def consume_reset_request(self) -> bool:
        """Return whether an episode reset was requested and clear the flag."""
        return self._viewer_ui_flag("consume_reset_request")

    def _viewer_ui_flag(self, method_name: str) -> bool:
        """Read an optional viewer UI flag, tolerating backends without that control.

        The Isaac Lab UI mixin adds reset/pause controls to Newton's own viewers, but external
        RTX backends (e.g. the OptiX path-tracing viewer) do not implement them and raise
        ``AttributeError`` from ``ViewerBase.__getattr__``. Missing controls simply mean the
        feature is unavailable, so report ``False`` instead of tearing the visualizer down.
        """
        if self._viewer is None:
            return False
    ...
    def is_training_paused(self) -> bool:
        ...
        if not self._is_initialized or self._viewer is None:
            return False
        return self._viewer_pause_state("is_training_paused")

    def is_rendering_paused(self) -> bool:
        """Return whether rendering is paused from viewer controls."""
        if not self._is_initialized or self._viewer is None:
            return False
        return self._viewer_pause_state("is_rendering_paused")

    def _viewer_pause_state(self, method_name: str) -> bool:
        """Read a viewer pause flag, tolerating viewers without the split pause API.

        Newton's GL viewer exposes separate ``is_training_paused`` / ``is_rendering_paused``
        controls, but third-party RTX backends (e.g. the OptiX path-tracing viewer) only expose
        the single ``paused`` property. Fall back to that, then to ``False``, so a missing
        control never tears the visualizer down mid-run.
        """
        method = getattr(self._viewer, method_name, None)
        if callable(method):
            return bool(method())
        return bool(getattr(self._viewer, "paused", False))
...
class NewtonOptixVisualizer(NewtonVisualizer):
    """External OptiX path-tracing visualizer for Isaac Lab Newton scenes."""

    def __init__(self, cfg: NewtonOptixVisualizerCfg):
        super().__init__(cfg)
        self.cfg: NewtonOptixVisualizerCfg = cfg

    def _create_viewer(self, runtime_headless: bool, metadata: dict):
        del metadata
        ...
        viewer = ViewerOptix(
            ...
            height=self.cfg.window_height,
            headless=runtime_headless,
            enable_imgui=not runtime_headless,
            enable_dlss_rr=self.cfg.enable_dlss_rr,
            dlss_quality=self.cfg.dlss_quality,
            samples_per_frame=self.cfg.samples_per_frame,
            max_bounces=self.cfg.max_bounces,
            direct_light_samples=self.cfg.direct_light_samples,
            time_of_day=self.cfg.time_of_day,
            sky_azimuth=self.cfg.sky_azimuth,
            sky_intensity=self.cfg.sky_intensity,
            exposure=self.cfg.exposure,
            contrast=self.cfg.contrast,
            saturation=self.cfg.saturation,
        )

        # ``PathTracingViewerBackend.__init__`` does not forward ``enable_cuda_graphs`` (only
        # ``PathTracerAPI`` accepts it), so configure the renderer it built. Needed when the Newton
        # physics CUDA graph shares the stream and OptiX capture fails after warm-up.
        if not self.cfg.enable_cuda_graphs:
            renderer = getattr(getattr(viewer, "_api", None), "viewer", None)
            if renderer is None:
                raise RuntimeError(
                    "Cannot disable OptiX CUDA graphs: ViewerOptix exposes no path-tracing renderer."
                )
            renderer.enable_cuda_graphs = False
    ...
    def _apply_camera_pose(
        ...
        if self._viewer is None:
            return
        cam_pos, cam_target = pose
        self._viewer.set_camera_look_at(cam_pos, cam_target, force=True)
        self._last_camera_pose = (cam_pos, cam_target)
    ...
    def render_rgb_array(self) -> np.ndarray | None:
        """Render and return the latest OptiX RGB frame."""
        if self._viewer is None:
            return None
        if self._runtime_headless and self._state is not None and not self._viewer.is_paused():
            self._viewer.begin_frame(self._sim_time)
            try:
                ...
                self._viewer.end_frame()
        frame = self._viewer.get_frame().numpy()
        # The OptiX output buffer has a bottom-left origin. ``ViewerOptix.get_frame`` documents a
        # top-left origin but does not flip, which is why the path tracer's own recording path uses
        # ``-vf vflip`` and its screenshot path uses ``np.flipud``. Isaac Lab's RGB contract is
        # top-left origin, so flip here; otherwise recorded videos come out upside down.
